...
t.Helper()
dir := t.TempDir()
bin := filepath.Join(dir, "salja")
cmd := exec.Command("go", "build", "-o", bin, "../../../cmd/salja")
cmd.Dir = filepath.Join(dir)
// Use the project root
projRoot, _ := filepath.Abs("../../..")
cmd.Dir = projRoot
cmd.Args = []string{"go", "build", "-o", bin, "./cmd/salja"}
...
if !strings.Contains(string(out), "salja version") {
