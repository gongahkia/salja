...
func (p *OutlookParser) Parse(ctx context.Context, r io.Reader, sourcePath string) (*model.CalendarCollection, error) {
	...
	for {
		...
		lineNum++
		item, err := parseOutlookRow(row, colMap, ec, sourcePath, lineNum)
		if err != nil {
	...
	if len(ec.Warnings) > 0 {
		for _, w := range ec.Warnings {
			fmt.Fprintf(os.Stderr, "outlook parser: %s\n", w)
		}
	}
...
func parseOutlookRow(row []string, colMap map[string]int, ec *salerr.ErrorCollector, sourcePath string, lineNum int) (model.CalendarItem, error) {
